...
- [ ] 1.1 Move the marker list that identifies a spent quota, credit or billing state into
...
- [ ] 1.2 Make `src/image.py` import that helper instead of carrying its own copy; verify
...
- [ ] 2.1 Add the branch to `llm._post` ahead of the transient-status block: mark the
  ...
  answers.
- [ ] 2.2 Make an exhausted-by-credit chain raise `LlmOverloadedError` with the model
  names it tried, like the quota case; verify a test asserts the class and that the
  message names the chain.
- [ ] 2.3 Keep a pinned-model call (the discovery probe) failing without walking the
  chain; verify a test asserts the probe still raises rather than marking other models.
- [ ] 2.4 Verify with a test that a `403` naming no credit, billing or quota reason still
  fails immediately as today, without touching the chain.

## 3. The import is parked, not discarded

- [ ] 3.1 Verify with a test through the import flow that a spent-credit chain parks the
  entry with a due time instead of failing it, and that the notification is the queued
  wording rather than the "no recipe" one.
- [ ] 3.2 Verify with a test that the parked entry completes on a later attempt once the
